...
def lambda_handler(event, context):
    ...
    try:
        ...
        for record in event["Records"]:
            ...
            logger.info(
                f"REQUEST_ID: {request_id} - Processing image: {key} from bucket: {bucket}, image_size: {file_size} bytes"
            )

            # Log large image warning
            if file_size > 10 * 1024 * 1024:  # 10 MB
                logger.warning(
                    f"REQUEST_ID: {request_id} - Large image detected: {key} ({file_size} bytes)"
                )
            ...
            logger.info(
                f"REQUEST_ID: {request_id} - Downloaded in {download_time:.2f}ms"
            )
            ...
            logger.info(
                f"REQUEST_ID: {request_id} - processing_time: {process_time:.2f}ms"
            )
            ...
            logger.info(
                f"REQUEST_ID: {request_id} - Uploaded {len(processed_images)} variants in {upload_time:.2f}ms"
            )

            processed_count += len(processed_images)
            logger.info(
                f"REQUEST_ID: {request_id} - Successfully processed {len(processed_images)} variants of {key}"
            )
        ...
        logger.info(
            f"REQUEST_ID: {request_id} - COMPLETED - Total time: {total_time:.2f}ms, Processed {processed_count} images"
        )
        ...
        logger.error(
            f"REQUEST_ID: {request_id} - ERROR processing image after {error_time:.2f}ms: {str(e)}",
            exc_info=True,
        )
...
def process_image(image_data, original_key, request_id="unknown"):
    ...
    try:
        ...
        logger.info(
            f"REQUEST_ID: {request_id} - Original image: {width}x{height}, format: {original_format}"
        )
        ...
        if width > MAX_DIMENSION or height > MAX_DIMENSION:
            ...
            logger.info(
                f"REQUEST_ID: {request_id} - Resized to: {new_width}x{new_height}"
            )
        ...
        for variant in variants:
            ...
            logger.info(
                f"REQUEST_ID: {request_id} - Created variant: {output_key} ({save_format}, quality: {variant['quality']})"
            )
        ...
        logger.info(
            f"REQUEST_ID: {request_id} - Created thumbnail: {base_name}_thumbnail_{unique_id}.jpg"
        )
        ...
        logger.error(
            f"REQUEST_ID: {request_id} - Image processing failed: {str(e)}",
            exc_info=True,
        )
        raise
